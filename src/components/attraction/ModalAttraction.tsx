...
import type { Attraction, Country, AttractionType, BudgetOrigin, Currency, Period } from '@/types/Attraction'
import { COUNTRIES, ATTRACTION_TYPES, BUDGET_ORIGINS, PERIODS } from '@/config/constants'
...
interface ModalAttractionProps {
  ...
  isOpen: boolean
  onClose: () => void
  onSave: (attraction: Omit<Attraction, 'id' | 'createdAt' | 'updatedAt'>) => void
}
...
export function ModalAttraction({ attraction, isOpen, onClose, onSave }: ModalAttractionProps) {
  const [formData, setFormData] = useState<Partial<Attraction>>({
    ...
    budgetOrigin: attraction?.budgetOrigin || 'couple',
  ...
  const handleChange = (field: keyof Attraction, value: any) => {
    setFormData(prev => {
      ...
      // Auto-calcular preço em BRL quando mudar valor ou moeda
      ...
      // Auto-calcular dia da semana quando mudar a data
      if (field === 'date' && value) {
  ...
  const handleSubmit = () => {
    onSave(formData as Omit<Attraction, 'id' | 'createdAt' | 'updatedAt'>)
  }
      ...
      <div className="space-y-6">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Origem do Pagamento *</label>
                <CustomSelect
                  value={formData.budgetOrigin ? `${BUDGET_ORIGINS[formData.budgetOrigin as BudgetOrigin].icon} ${BUDGET_ORIGINS[formData.budgetOrigin as BudgetOrigin].label}` : ''}
                  onChange={(val) => {
                    const originKey = Object.entries(BUDGET_ORIGINS).find(([_, o]) => `${o.icon} ${o.label}` === val)?.[0]
                    if (originKey) handleChange('budgetOrigin', originKey as BudgetOrigin)
                  }}
                  options={Object.entries(BUDGET_ORIGINS).map(([_, origin]) => `${origin.icon} ${origin.label}`)}
                />
              </div>
